...
class InteractionModelParticleTable(metaclass=_ABCMeta):
    ...
    def __init__(self):
        ...
        for modname, pids in part_table.items():
            mod_id, pdg_id = pids
        ...
        assert (
            len(list(self.modname2pdg.keys())) == len(set(self.modname2pdg.keys()))
...
class SibyllParticleTable(InteractionModelParticleTable):
    ...
    def __init__(self):
        ...
        self.baryon_range = []
        temp_dict = {}
        for name, (modid, pdgid) in self.part_table.items():
...
class QGSJetParticleTable(InteractionModelParticleTable):
    ...
    def __init__(self):
        ...
        self.baryon_range = []
        temp_dict = {}
        for (modid, pdgid) in self.part_table.values():
...
def print_stable(life_time_greater_then=1e-10):
    ...
    for pname in pyth_data.pytname2data.keys():
...
def test():
    ...
    for pname, pvalues in pyth_data.pytname2data.items():
        if pname.find('~') == -1:
